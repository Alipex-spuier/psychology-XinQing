<template>
	<view class="">
		<view class="qu-bar">
			<view class="qu-bar-wrapper" v-if="isLoaded">
			...
			getArtcles() {
				  //return new Promise((resolve, reject) => {
					...
					success: (response) => {
						...
						let randomArticles = this.getRandomItems(articles, articles.length);
						for(let i=0;i<articles.length;i++){
							let tmp={}
							tmp.albumCover = randomArticles[i].artPic;
							tmp.title = randomArticles[i].artTitle;
